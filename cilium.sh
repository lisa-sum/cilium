...
kubectl -n kube-system exec pod/cilium-5g2k4 -- cilium status --verbose
...
--set externalTrafficPolicy=Cluster \ # 对于创建服务时默认的 Cluster 策略，存在多个选项来实现外部流量的客户端源 IP 保留，即，如果后者仅向外界公开基于 TCP 的服务，则在 DSR 或混合模式下操作 kube-proxy 替换。
